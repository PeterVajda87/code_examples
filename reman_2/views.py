...
class ReconditionMapSimple:
    def __init__(self, period_start, period_end):
        self.period_start = period_start
        self.period_end = period_end
        self.orders = Header.objects.filter(release_date__gte=self.period_start, release_date__lte=self.period_end, order_type='YRM2').values_list('order', flat=True)
    ...
    def add_order(self, component_object):
        ...
        if not sm.material_number in self.results:
            self.results[sm.material_number] = {}
    
        if not fm.material_number in self.results[sm.material_number]:
            self.results[sm.material_number][fm.material_number] = [component_object.quantity_withdrawn, Header.objects.get(order=component_object.order).quantity_delivered, Header.objects.get(order=component_object.order).quantity_delivered / component_object.quantity_withdrawn, dict()]
        else:
            self.results[sm.material_number][fm.material_number][0] += component_object.quantity_withdrawn
            self.results[sm.material_number][fm.material_number][1] += Header.objects.get(order=component_object.order).quantity_delivered
            self.results[sm.material_number][fm.material_number][2] = self.results[sm.material_number][fm.material_number][1] / self.results[sm.material_number][fm.material_number][0]

        losses = list(Losses.objects.filter(order=Header.objects.get(order=component_object.order).order).values('damage', 'quantity'))

        for loss in losses:
            if loss['damage'] in self.results[sm.material_number][fm.material_number][3]:
                self.results[sm.material_number][fm.material_number][3][loss['damage']] += loss['quantity']
            else:
                self.results[sm.material_number][fm.material_number][3][loss['damage']] = loss['quantity']
    ...
    def to_json(self):
        return json.dumps(self.results, cls=SetEncoder)
...
class ReconditionMap:
    ...
    def add_order(self, component_object):
        ...
        if not fm.material_number in self.materials:
            self.materials[fm.material_number] = {}
            self.materials[fm.material_number]['qty_as_input_material'] = {}
            self.materials[fm.material_number]['qty_as_output_material'] = 0
            self.materials[fm.material_number]['parent_materials'] = set()
            self.materials[fm.material_number]['children_materials'] = set()
        if not sm.material_number in self.materials:
            self.materials[sm.material_number] = {}
            self.materials[sm.material_number]['qty_as_input_material'] = {}
            self.materials[sm.material_number]['qty_as_output_material'] = 0
            self.materials[sm.material_number]['parent_materials'] = set()
            self.materials[sm.material_number]['children_materials'] = set()

        self.materials[component_object.pegged_requirement.material_number]['qty_as_output_material'] += component_object.quantity_withdrawn
        self.materials[component_object.pegged_requirement.material_number]['parent_materials'].add(component_object.material.material_number)
        if component_object.pegged_requirement.material_number in self.materials[component_object.material.material_number]['qty_as_input_material']:
            self.materials[component_object.material.material_number]['qty_as_input_material'][component_object.pegged_requirement.material_number] += component_object.requirement_quantity
        else:
            self.materials[component_object.material.material_number]['qty_as_input_material'][component_object.pegged_requirement.material_number] = component_object.requirement_quantity

        self.materials[component_object.material.material_number]['children_materials'].add(component_object.pegged_requirement.material_number)
    ...
    def process_results(self):
        for material in self.materials:
            material_object = Material.objects.get(material_number=material)
            if self.is_root_recondition_material(material_object):
                if not self.materials[material]['parent_materials']:
                    self.results[material] = {}
                    self.results[material]['meta'] = {}
                    self.results[material]['meta']['level'] = 0
                    self.results[material]['meta']['as_input'] = self.materials[material]['qty_as_input_material']
                    for children_material in self.materials[material]['children_materials']:
                        self.results[material][children_material] = {}
                        self.results[material][children_material]['meta'] = {}
                        self.results[material][children_material]['meta']['level'] = 1
                        self.results[material][children_material]['meta']['as_input'] = self.materials[children_material]['qty_as_input_material']
                        self.results[material][children_material]['meta']['as_output'] = self.materials[children_material]['qty_as_output_material']
                        if self.materials[children_material]['children_materials']:
                            for grandchildren_material in self.materials[children_material]['children_materials']:
                                self.results[material][children_material][grandchildren_material] = {}
                                self.results[material][children_material][grandchildren_material]['meta'] = {}
                                self.results[material][children_material][grandchildren_material]['level'] = 2
                                self.results[material][children_material][grandchildren_material]['as_input'] = self.materials[grandchildren_material]['qty_as_input_material']
                                self.results[material][children_material][grandchildren_material]['as_output'] = self.materials[grandchildren_material]['qty_as_output_material']
                                if self.materials[grandchildren_material]['children_materials']:
                                    for grandgrandchildren_material in self.materials[grandchildren_material]['children_materials']:
                                        self.results[material][children_material][grandchildren_material][grandgrandchildren_material] = {}
                                        self.results[material][children_material][grandchildren_material][grandgrandchildren_material]['meta'] = {}
                                        self.results[material][children_material][grandchildren_material][grandgrandchildren_material]['level'] = 3
                                        self.results[material][children_material][grandchildren_material][grandgrandchildren_material]['as_input'] = self.materials[grandgrandchildren_material]['qty_as_input_material']
                                        self.results[material][children_material][grandchildren_material][grandgrandchildren_material]['as_output'] = self.materials[grandgrandchildren_material]['qty_as_output_material']
...
class DisassemblyMap:
    def __init__(self, period_start, period_end):
        ...
        self.period_start = period_start
        self.period_end = period_end
        self.q3_processed_cm_materials = []
        self.orders_in_period = Header.objects.filter(release_date__gte=self.period_start, release_date__lte=self.period_end).values_list('order', flat=True)
    ...
    def process_cm_material_group(self, rm, cm, order, component_object, cm_quantity):
        ...
        if not cm_material_group in self.map[customer_name]:
            self.map[customer_name][cm_material_group] = {}
            self.map[customer_name][cm_material_group]['no_of_different_cm'] = 0
            self.map[customer_name][cm_material_group]['qty_in'] = order.order_quantity
            self.map[customer_name][cm_material_group]['qty_wanted'] = order.order_quantity * cm_quantity
            self.map[customer_name][cm_material_group]['qty_out'] = -component_object.quantity_withdrawn
            self.map[customer_name][cm_material_group]['table_rows'] = 0
        else:
            self.map[customer_name][cm_material_group]['qty_in'] += order.order_quantity
            self.map[customer_name][cm_material_group]['qty_wanted'] += order.order_quantity * cm_quantity
            self.map[customer_name][cm_material_group]['qty_out'] += -component_object.quantity_withdrawn
    ...
    def process_component_material(self, rm, cm, order, component_object, cm_quantity):
        ...
        if not cm.material_number in self.map[customer_name][cm_material_group]:
            self.map[customer_name]['table_rows'] += 1
            self.map[customer_name][cm_material_group]['table_rows'] += 1
            self.map[customer_name][cm_material_group]['no_of_different_cm'] += 1
            self.map[customer_name][cm_material_group][cm.material_number] = {}
            self.map[customer_name][cm_material_group][cm.material_number]['qty_in'] = order.order_quantity
            self.map[customer_name][cm_material_group][cm.material_number]['qty_wanted'] = order.order_quantity * cm_quantity
            self.map[customer_name][cm_material_group][cm.material_number]['qty_out'] = -component_object.quantity_withdrawn
            self.map[customer_name][cm_material_group][cm.material_number]['q3'] = {}
            self.map[customer_name][cm_material_group][cm.material_number]['q3']['damage_codes'] = {}
        else:
            self.map[customer_name][cm_material_group][cm.material_number]['qty_in'] += order.order_quantity
            self.map[customer_name][cm_material_group][cm.material_number]['qty_wanted'] += order.order_quantity * cm_quantity
            self.map[customer_name][cm_material_group][cm.material_number]['qty_out'] += -component_object.quantity_withdrawn

        if not cm in self.q3_processed_cm_materials:
            self.q3_processed_cm_materials.append(cm)
            if Losses.objects.filter(assembly=cm, order__in=self.orders_in_period).exists():
                q3 = Losses.objects.filter(assembly=cm, order__in=self.orders_in_period)

                for order in q3:
                    self.map[customer_name][cm_material_group][cm.material_number]['q3'][order.order] = {}
                    self.map[customer_name][cm_material_group][cm.material_number]['q3'][order.order]['quantity'] = order.quantity

                    if order.damage in self.map[customer_name][cm_material_group][cm.material_number]['q3']['damage_codes']:
                        self.map[customer_name][cm_material_group][cm.material_number]['q3']['damage_codes'][order.damage] += order.quantity
                    else:
                        self.map[customer_name][cm_material_group][cm.material_number]['q3']['damage_codes'][order.damage] = order.quantity
        ...
        try:
            self.map[customer_name][cm_material_group][cm.material_number]['rate_component_material'] = self.map[customer_name][cm_material_group][cm.material_number]['qty_out'] / self.map[customer_name][cm_material_group][cm.material_number]['qty_wanted']
        except:
            self.map[customer_name][cm_material_group][cm.material_number]['rate_component_material'] = 0

        try:
            self.map[customer_name][cm_material_group]['rate_cm_group'] = (self.map[customer_name][cm_material_group]['qty_out'] / self.map[customer_name][cm_material_group]['qty_wanted'])
        except:
            self.map[customer_name][cm_material_group]['rate_cm_group'] = 0
...
def disassembly(request):
    ...
    components = Component.objects.filter(order__in=orders_in_period).exclude(pegged_requirement=F('material')).filter(quantity_withdrawn__lt=0)

    #### Pokud je soucet vsech komponentu 0, tak zakazku nepocitat ###
...
def recondition(request):
    if request.method == "GET":
        ...
        recondition_map = ReconditionMapSimple(period_start, period_end)
        
        for component in components:
            if component.quantity_withdrawn > 0:
